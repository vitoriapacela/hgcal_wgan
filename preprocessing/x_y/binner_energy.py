...
def process(rechit_x, rechit_y, rechit_layer, rechit_energy, seed_x, seed_y):
    eta_low_edge = seed_x - HALF_ETA
    eta_bins = np.floor((rechit_x - eta_low_edge) / BIN_WIDTH_ETA).astype(np.int32)

    phi_low_edge = seed_y - HALF_PHI
    phi_bins = np.floor((rechit_y - phi_low_edge) / BIN_WIDTH_PHI).astype(np.int32)
