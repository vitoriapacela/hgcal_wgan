...
def reshapeData(inp):
    '''
    Function to reshape the data in the data generator.
    :param inp: array with shape (n, 16, 16, 55, 1)
    :type inp: numpy array
    :return: numpy array with shape (n, 16, 16, 55)
    '''
    inp = inp[:, :, :, :, 0]
    return inp
        
def plotPred(real_sum, generated_sum, step):
    '''
    Plots histogram of the total energy of true samples and of generated samples.
    :param real_sum: (numpy array) total energy of true samples.
    :param generated_sum: (numpy array) total energy of generated samples.
    ...
    #plt.show()
    plt.savefig('images/pred_%s.png'%step)
...
class WGAN():
    ...
    def wasserstein_loss(self, y_true, y_pred):
        '''
        Computes the mean of the difference between true samples and prediction samples.
        :param y_true: tensor variable of true values.
        :param y_pred: tensor variable of predictions.
        '''
    ...
    def train(self, steps, batch_size=128, sample_interval=50):
        '''
        Trains model.
        :param steps: (int) number of steps.
        :batch_size: (int) batch size.
        :sample_interval: (int) number of steps before sampling and saving.
        '''
...
#         f.close()
        ...
        # Load the training dataset with data generator
        train_dir = '/bigdata/shared/HGCAL_data/new_multi_small/no_pu/'
        
        train = gen_from_data(train_dir, batch_size=batch_size, data_keys="X", prep_func=reshapeData)
...
#                 imgs = X_train[idx]
            ...
            if step % sample_interval == 0:
                ...
                self.generator.save_weights("weights/generator_weights_step_%d_%s.h5" % (step, self.tag))
                self.critic.save_weights("weights/discriminator_weights_step_%d_%s.h5" % (step, self.tag))
                self.combined.save_weights("weights/combined_weights_step_%d_%s.h5" % (step, self.tag))
                ...
                #plotPred(inp_sum[:,0], gen_sum[:,0], step)
    ...
    def sample_images(self, step):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("images/hgcal_%d.png" % step)
        plt.close()
...
if __name__ == '__main__':
    ...
    wgan = WGAN()
    wgan.train(steps=3, batch_size=16, sample_interval=25)
